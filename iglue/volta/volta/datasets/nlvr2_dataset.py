...
class NLVR2Loader(object):
    def __init__(
        ...
        if self.number_of_entries:
            ...
            ds = td.FixedSizeData(ds, self.number_of_entries, keep_state=False)
        else:
            ds = td.LMDBSerializer.load(lmdb_file, shuffle=False)
        self.num_dataset = len(ds)

        print("len dataset = ")
        print(self.num_dataset)
...
class BertPreprocessBatch(object):
    def __init__(
        ...
        if valid_pos_tags and caption_to_pos_tag_file and en_to_lang_dict_file:
        ...
        if en_to_lang_dict_file:
            with open(en_to_lang_dict_file,'rb') as handle:
                self.en_to_lang_dict = pkl.load(handle)
    ...
    def __call__(self, item):
        ...
        if self.code_switching:
            # print("old sentence = ")
            # print(sentence)
            sentence = self.code_switch_sentence(sentence)
            # print("new caption =")
            # print(sentence)
            # print("\n")
    ...
    def code_switch_sentence(self, sentence):
        """
        Given a sentence, will perform code-switching and translate some of the words to another language.
        """

        if "\n" in sentence:
            sentence = sentence.replace("\n", " ")

        if sentence in self.caption_to_pos_tag:
            pos_tokens = self.caption_to_pos_tag[sentence]
            ...
            for init_word, pos in pos_tokens:
                ...
                if pos in self.valid_pos_tags:
                    ...
                    if dict_pair in self.en_to_lang_dict:
                        if random.random() < 0.5:
                            new_word = self.en_to_lang_dict[dict_pair]
