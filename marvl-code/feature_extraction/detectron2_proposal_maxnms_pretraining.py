...
def doit(raw_image, predictor):
    with torch.no_grad():
        ...
        instances.pred_class_logits = pred_class_logits[ids].detach()
        instances.attr_logits = pred_attr_logits[ids].detach()
...
def extract(output_fname, dataloader, desc, caption_path):
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(dataloader),
                ...
                try:
                    ...
                    grp['pred_class_logits'] = instances.pred_class_logits.numpy()
                    ...
                    grp['attr_logits'] = instances.attr_logits.numpy()
                    grp['boxes'] = instances.pred_boxes.tensor.numpy()
                    grp['num_boxes'] = num_objects
                    grp['img_w'] = img.shape[1]
                    grp['img_h'] = img.shape[0]
                    grp['imag_id'] = img_id
                    grp['caption'] = captions[str(img_id)]

 
                except Exception as e:
                    print("Probably wrong caption file")
                    print(e)
                    continue
