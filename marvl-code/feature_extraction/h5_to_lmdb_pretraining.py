...
if __name__ == '__main__':
    ...
    parser.add_argument('--indexes', type=str, default="all")
    args = parser.parse_args()

    source_fname = args.h5
    target_fname = args.lmdb
    indexes_fname = args.indexes

    if indexes_fname != "all":
        with open(indexes_fname, 'rb') as handle:
            selected_indexes = pickle.load(handle)
    ...
    with env.begin(write=True) as txn:
        ...
        for i, img_id in enumerate(tqdm(f.keys(), ncols=100)):
            ...
            if indexes_fname != "all" and img_id not in selected_indexes:
                continue
