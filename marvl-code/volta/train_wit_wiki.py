...
from volta.datasets import WITLoaderTrain, WITLoaderVal, WikipediasDataset
...
def parse_args():
    ...
    # Code Switching
    parser.add_argument("--valid_pos_tags", nargs='+', default=None,
                        help="POS tags to consider for code-switching, e.g. NOUN.")
    parser.add_argument("--caption_to_pos_tag_file", default=None, type=str,
                        help="Mapping for every caption to a list of (word, pos tag) pairs.")
    parser.add_argument("--lang_to_en_dict_file", default=None, type=str,
                        help="Mapping from (lang word, pos tag) -> en word.")
    parser.add_argument("--word_to_reduce_version_file", default=None, type=str,
                        help="Mapping from words to reduced version (lemmatized for e.g), that is included in the"
                         "lang_to_en_dict. It helps increase the code-switch coverage on some languages.")
    parser.add_argument("--add_spacing_code_switching", nargs='?', default=False, const=True,
                        help="Used to add a spacing around the code-switched word. It needs to be used for Chinese"
                        "where characters are not spaced.")
...
def main():
    ...
    # Datasets
    # x_tokenizer = BertTokenizer.from_pretrained(args.bert_model, do_lower_case=args.do_lower_case)
    ...
    print("add_spacing_code_switching =")
    print(args.add_spacing_code_switching)


    train_x_dataset = WITLoaderTrain(args.annotations_path, args.features_path, tokenizer, args.bert_model,
                                            ...
                                            add_global_imgfeat=config.add_global_imgfeat, num_locs=config.num_locs,
                                            valid_pos_tags=args.valid_pos_tags, caption_to_pos_tag_file=args.caption_to_pos_tag_file,
                                            lang_to_en_dict_file=args.lang_to_en_dict_file, 
                                            word_to_reduce_version_file=args.word_to_reduce_version_file,
                                            add_spacing_code_switching=args.add_spacing_code_switching)
    valid_x_dataset = WITLoaderVal(args.annotations_path, args.features_path, tokenizer, args.bert_model,
                                          seq_len=args.max_x_seq_length, batch_size=args.train_x_batch_size,
                                          num_workers=2, objective=args.objective, tokenizer_name=args.m_pretrained,
                                          add_global_imgfeat=config.add_global_imgfeat, num_locs=config.num_locs)
                                          
    # Task details
    task_names = ["WIT+Wikipedia"]
    task_ids = ["TASK00"]
    ...
    # model_test = BertForVLPreTraining(config)
    # state_dict_test = model_test.load_state_dict(torch.load("/mnt/localdata/jouve/marvl/muniter/pytorch_model_9.bin"))
    # print("model test len =")
    # print(len(state_dict_test.keys()))
    ...
    if args.x_pretrained:
        ...
        print("x state dicts keys =")
        print(len(x_state_dict.keys()))

        keys_original = state_dict.keys()
        keys_pretrained = x_state_dict.keys()

        for key in keys_original:
            if not key in keys_pretrained:
                print("key missing  in pretrained model = {k}".format(k = key))

        count = 0
        for key, value in x_state_dict.items():
    ...
    # TODO: I added the score to make the code run.T

    # Save starting model
    save(save_path, logger, -1, model, optimizer, scheduler, global_step, tb_logger, default_gpu, 0)
    ...
    for epoch_id in range(start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_x_dataset):
            ...
            if n_gpu > 1:
                ...
                #if step % 20 == 0:
                #    print("training: loss = {l1} | pair_match_loss = {l2}".format(l1 = loss, l2 = pair_match_loss))
            ...
            if (step + 1) % args.grad_acc_steps == 0:
                ...
                if default_gpu:
                    tb_logger.step_train_CC(epoch_id, iter_id,
                                            float(masked_loss_t), float(masked_loss_v), float(pair_match_loss) * 100,
        ...
        for step, batch in enumerate(valid_x_dataset):
            ...
            if default_gpu:
                tb_logger.step_val_CC(iter_id, float(masked_loss_t), float(masked_loss_v), float(pair_match_loss) * 100,
                                      "TASK00", batch_size, "val")
                sys.stdout.write("%d / %d \r" % (step, numBatches))
                sys.stdout.flush()

        torch.set_grad_enabled(True)
        save(save_path, logger, epoch_id, model, optimizer, scheduler, global_step, tb_logger, default_gpu, 0)
