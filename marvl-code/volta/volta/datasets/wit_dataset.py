...
# with open("/mnt/nas_home/mrgj4/wit_dataset/multiling/captions/caption_train.json") as f:
#     captions_train = json.load(f)

# with open("/mnt/nas_home/mrgj4/wit_dataset/multiling/captions/caption_valid.json") as f:
#     captions_valid = json.load(f)
...
class WITLoaderTrain(object):
    ...
    def __init__(
        ...
        valid_pos_tags=None,
        caption_to_pos_tag_file=None,
        lang_to_en_dict_file=None,
        word_to_reduce_version_file=None,
        add_spacing_code_switching=False,
    ):
        ...
        preprocess_function = BertPreprocessBatch(
            ...
            word_to_reduce_version_file=word_to_reduce_version_file,
            add_spacing_code_switching=add_spacing_code_switching,
        )
...
class BertPreprocessBatch(object):
    def __init__(
        ...
        self.code_switching = False
        self.add_spacing_code_switching = add_spacing_code_switching
        self.word_to_reduce_version_file = word_to_reduce_version_file

        if valid_pos_tags and caption_to_pos_tag_file and lang_to_en_dict_file:
            self.code_switching = True

        print("code_switching =")
        print(self.code_switching)
        print("split =")
        print(self.split)
        print("caption path = ")
        print(caption_path)   
        print("add_spacing_code_switching = ")
        print(self.add_spacing_code_switching)
        print("valid pos tag =")
        ...
        if valid_pos_tags:
            self.valid_pos_tags = set(valid_pos_tags)
        if caption_to_pos_tag_file:
            with open(caption_to_pos_tag_file,'rb') as handle:
                self.caption_to_pos_tag = pkl.load(handle)
        if lang_to_en_dict_file:
            with open(lang_to_en_dict_file,'rb') as handle:
                self.lang_to_en_dict = pkl.load(handle)
        if word_to_reduce_version_file:
            with open(word_to_reduce_version_file,'rb') as handle:
                self.word_to_reduce_version = pkl.load(handle)
    ...
    def __call__(self, data):
        ...
        image_feature_wp = data['features']
        image_cls_wp = data['pred_class_logits']
        obj_labels = data['obj_id']
        obj_confs = data['obj_conf']
        attr_labels = data['attr_id']
        attr_confs = data['attr_conf']
        attr_scores = data['attr_logits']
        image_location_wp = data['boxes']
        num_boxes = data['num_boxes']
        image_h = data['img_h']
        image_w = data['img_w']
        image_id = data['imag_id']
        caption = data['caption']

        # quick fix
        # if str(image_id) in captions_train:
        #     caption = captions_train[str(image_id)]
        # else:
        #     caption = captions_valid[str(image_id)]
        ...
        if self.code_switching:
            # print("old caption = ")
            # print(caption)
            caption = self.code_switch_caption(caption)
            # print("new caption =")
            # print(caption)
            # print("\n")
    ...
    def code_switch_caption(self, caption):
        """
        ...
        def perform_dictionary_switch(init_word, dict_pair, caption):
            if dict_pair in self.lang_to_en_dict:
                if random.random() < 0.5:
                    new_word = self.lang_to_en_dict[dict_pair]
                    if init_word[0].isupper():
                        new_word = new_word.capitalize()
                    if self.add_spacing_code_switching:
        ...
        if caption in self.caption_to_pos_tag:
            pos_tokens = self.caption_to_pos_tag[caption]

            for init_word, pos in pos_tokens:

                if pos in self.valid_pos_tags:
                    word = init_word.lower()
                    dict_pair = (word, pos)

                    caption = perform_dictionary_switch(init_word, dict_pair, caption)

                    if self.word_to_reduce_version_file:
                        if word in self.word_to_reduce_version:
                            reduced_word = self.word_to_reduce_version[word]
